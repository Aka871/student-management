...
ポートフォリオとして作成した、受講生情報と受講コース情報を管理するWeb APIアプリケーションです。  
現在はAPIのみの構成で、各機能の動作確認には主にPostmanを使用しています。<br>
また、SwaggerUIでAPI仕様やリクエスト・レスポンスの形式を確認できます。<br>
Java / Spring Boot / MyBatis / MySQLを使用し、API仕様書、バリデーション、例外処理なども実装済みです。
...
本アプリは、スクール運営における以下のような管理上の課題を想定し、開発しています。
...
本アプリでは、受講生情報とコース情報を一元的に扱い、管理しやすくすることを目指しました。
...
| 機能名                         | 詳細                                                                                                   |
|--------------------------------|--------------------------------------------------------------------------------------------------------|
| 受講生詳細【登録】             | 受講生情報と受講生コース情報をセットで登録します（受講生IDはUUIDで自動生成）                           |
...
- 受講生詳細情報：受講生情報 + 受講生コース情報 を組み合わせた情報（1対多の関係）
...
> **Note**  
> 初期データは動作確認用のサンプルです。受講生IDのUUID は説明用に固定値を使用しています。  
> 実際にアプリケーションから受講生情報を登録する場合、受講生IDはUUIDで自動生成されます。
...
`students_courses` は、受講生IDで受講生情報と紐づけ、受講生ごとのコース情報（コース名、開始日、終了予定日など）を管理するテーブルです。
...
- **複数コース受講への対応**<br>
  1人の受講生が複数のコースを受講できるように、受講生IDで紐づけてコース情報を管理する `students_courses` テーブルを作成しました。  
  また、受講生情報と、その受講生に紐づく複数のコース情報を1つのレスポンスとして返せるように、`StudentDetail` クラスを用意しました。
...
- **バリデーションと例外レスポンスの整備**<br>
  未入力や形式エラーに対するチェック（電話番号やメール形式など）を実装し、例外発生時には、HTTPステータスとエラーメッセージを返すようにしました。


- **論理削除の導入**<br>
  データを物理的に削除せず、`isDeleted`フラグを更新することで、削除済みとして扱う仕組みを実装しました。  
  受講生詳細情報の一覧取得では、`GET /students`で論理削除済みの受講生を除外し、`GET /students/details`で論理削除済みを含む、すべての受講生の詳細情報を取得できるように、用途に応じてエンドポイントを分けています。
...
- **例外処理の追加**<br>
  `/courses`で指定したコース名が存在しない場合に、`CourseNotFoundException`を発生させ、エラーレスポンスを返す仕様とするかどうかを、利用目的に応じて検討します。


- **受講生コース情報テーブルの設計見直し**<br>
  現在の`students_courses`テーブルには主キーを設定していないため、今後は受講情報1件ごとに一意のIDを付与する方法や、`student_id`と`course_id`を組み合わせた複合主キーを設定する方法を、再受講などの要件も踏まえて検討します。
...
- **Dockerによる環境構築**<br>
  Dockerを導入し、アプリケーションとMySQLをコンテナ化することで、簡単に環境構築を行えるようにする予定です。
